...
def send_init_sequence(udp_socket, frame_number, uuid, chunk_size, total_chunks, server_address):
    ...
    logging.info('Successfully sent init message to server')
...
def send_end_sequence(udp_socket, frame_number, uuid, frame_size, combined_checksums, server_address, error=False):
    ...
    message = header + payload
    udp_socket.sendto(message, server_address)
    logging.info('Successfully sent end message to server')


def send_frame(frame_number, udp_socket, frame, server_address):
    """Gửi một frame qua UDP và nhận phản hồi từ server."""
    ...
    for sequence in range(1, total_chunks + 1):
        ...
        payload = chunk
        checksum = calculate_checksum(frame_number, sequence, payload, uuid)
...
def process_frame_queue():
    ...
    while not stop_event.is_set(): 
        ...
        try:
            for i in range(30):
                frame_queue.put(byte_content)  # Đẩy dữ liệu vào hàng đợi
                # logging.info(f"Frame {i} put into queue. Queue size: {frame_queue.qsize()}")  # Log số lượng phần tử
            time.sleep(0.5)
        except Exception as e:
