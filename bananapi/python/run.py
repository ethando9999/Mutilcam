...
from send_feature import start_sender
from processing import start_processor
# from put_frame import start_putter
from logging_python_bananapi import setup_logging, get_logger
...
async def main():
    ...
    try:
        # Khởi động server trước để nhận frame từ RPI
        server_task = asyncio.create_task(start_server(frame_queue))
        ...
        processor_task = asyncio.create_task(
            start_processor(
                ...
                num_workers=2  # Số lượng processor workers
            )
        )
