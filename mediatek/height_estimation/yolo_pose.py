...
class HumanDetection:
    ...
    def calculate_head_size(self, keypoints):
        """
        Tính kích thước đầu (head size) từ keypoints của một người.

        Args:
            keypoints (np.ndarray): Array of shape (17, 2) containing (x, y) coordinates of 17 keypoints.

        Returns:
            float: Head size in pixels, or None if insufficient data or any coordinate contains 0.
        """
        # Check if there are enough keypoints (at least nose and one eye)
        if len(keypoints) < 3:
            logger.warning("Không đủ keypoints để tính head size.")
            return None

        # Extract keypoints: 0 - nose, 1 - left eye, 2 - right eye
        nose = keypoints[0]
        left_eye = keypoints[1]
        right_eye = keypoints[2]

        # Check if any coordinate (x or y) is 0 in nose, left_eye, or right_eye
        if (0 in nose) or (0 in left_eye) or (0 in right_eye):
            logger.warning("Có tọa độ chứa giá trị 0 (nose, left_eye, hoặc right_eye). Trả về None.")
            return None

        # Calculate the center of the eyes
        eyes_center = (left_eye + right_eye) / 2

        # Calculate Euclidean distance between eyes center and nose
        # print(f"nose: {nose}, left_eye: {left_eye}, right_eye: {right_eye}")
        distance_px = np.linalg.norm(eyes_center - nose)

        # Compute head size using the formula
        head_size = distance_px * 5.7
        return head_size

    def extract_head_keypoints(self, keypoints):
        """
        Trích xuất các keypoints vùng đầu (nose, eyes, ears) từ mảng keypoints đầy đủ.

        Args:
            keypoints (np.ndarray): Array of shape (17, 2)

        Returns:
            list[tuple]: Danh sách các điểm vùng đầu [(x, y), ...], hoặc None nếu thiếu điểm.
        """
        head_indices = [0, 1, 2, 3, 4]  # nose, left_eye, right_eye, left_ear, right_ear

        try:
            head_kp = [tuple(keypoints[i]) for i in head_indices]
        except IndexError:
            logger.warning("Không đủ keypoints để trích xuất vùng đầu.")
            return None

        if any((x == 0 and y == 0) for (x, y) in head_kp):
            logger.warning("Một số keypoint vùng đầu không hợp lệ (0, 0).")
            return None
