...
images_left = sorted(glob.glob('calibration7/cam1/*.jpg'), key=natural_keys)
images_right = sorted(glob.glob('calibration7/cam2/*.jpg'), key=natural_keys) 
...
for left_img_path, right_img_path in zip(images_left, images_right):
    ...
    if ret_left and ret_right:
        ...
        # Resize ảnh trước khi hiển thị (chia 2 kích thước)
        img_left_resized = cv2.resize(img_left, (img_left.shape[1] // 2, img_left.shape[0] // 2))
        img_right_resized = cv2.resize(img_right, (img_right.shape[1] // 2, img_right.shape[0] // 2))

        # Hiển thị ảnh với các góc đã được đánh dấu
        cv2.imshow('Left Image', img_left_resized)
        cv2.imshow('Right Image', img_right_resized)

        cv2.waitKey(50)  # Chờ 500ms trước khi chuyển sang ảnh tiếp theo
