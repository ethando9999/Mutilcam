...
var (
	...
	sentFrames          = 0
	startTime           = time.Now()
	frameProcessingChan = make(chan struct{}, 20)
)
...
type Frame struct {
	...
	FeatureData      []byte      // Dữ liệu tính năng lưu dưới dạng byte
	mu               sync.Mutex
}
...
func checkLinuxRamDisk(ramdisk string) bool {
...
func checkUnixSocket(unixSocket string) bool {
...
func handleChunk(frameNumber byte, sequenceNumber uint16, checksum uint16, payload []byte, addr *net.UDPAddr, conn *net.UDPConn, port int) error {
	...
	// Kiểm tra độ dài payload (cho phép chunk cuối nhỏ hơn 128 bytes)
	if len(payload) < 128 && int(sequenceNumber) != frame.TotalChunks {
		return fmt.Errorf("invalid chunk payload: too short")
	}

	if _, exists := frame.ReceivedChecksum[int(sequenceNumber)]; exists {
		return fmt.Errorf("handleChunk - duplicate package for sequence %d with frameNumber: %d", sequenceNumber, frameNumber)
	}
	...
	if frame.EndChecksum != 0 && len(frame.ReceivedChecksum) == frame.TotalChunks {
		...
		err := processEndFrame(frameNumber, frame, uuid, addr, conn, port)
		return err
	}
...
func handleEnd(frameNumber byte, sequenceNumber uint16, checksum uint16, encryptedPayload []byte, addr *net.UDPAddr, conn *net.UDPConn, port int) error {
		...
		frame.TimeoutTimer = time.AfterFunc(1*time.Second, func() {
			...
			if len(frame.ReceivedChecksum) != frame.TotalChunks {
				...
				if len(missingChunks) > 0 {
					...
					log.Printf("Failed Frame Number: %d sent to %s", frameNumber, addr.String())
				}
				// frameMapMu.Lock()
	...
	return processEndFrame(frameNumber, frame, uuid, addr, conn, port)
}

func processEndFrame(frameNumber byte, frame *Frame, uuid string, addr *net.UDPAddr, conn *net.UDPConn, port int) error {
	...
	log.Printf("Server %d successfully received FrameNumber: %d", port, frameNumber)
...
// Sửa hàm sendToProcessor để tự động kết nối lại khi gặp lỗi
func sendToProcessor(frame []byte, featureData []byte, unixSocket string) error {
	// Tạo header và payload như cũ
	header := make([]byte, 8)
	binary.BigEndian.PutUint32(header[0:4], uint32(len(frame)))
	binary.BigEndian.PutUint32(header[4:8], uint32(len(featureData)))
	payload := append(append(header, frame...), featureData...)

	// Kết nối trực tiếp tới Unix socket
	conn, err := net.Dial("unix", unixSocket)
	if err != nil {
		return fmt.Errorf("không thể kết nối tới processor: %v", err)
	}
	defer conn.Close()
	...
	if err != nil {
		return fmt.Errorf("lỗi khi gửi dữ liệu: %v", err)
	}
	// log.Printf("Gửi dữ liệu tới processor thành công")

	return nil
}

// Sửa hàm processEndFeature để xử lý lỗi từ sendToProcessor
func processEndFeature(frameNumber byte, frame *Frame, unixSocket string) error {
	var buffer bytes.Buffer
	buffer.Grow(frame.TotalChunks * frame.ChunkSize)
	...
	// Xử lý lỗi từ sendToProcessor
	if err := sendToProcessor(buffer.Bytes(), frame.FeatureData, unixSocket); err != nil {
		log.Printf("Error in sendToProcessor for frame %d: %v", frameNumber, err)
		// Không return ở đây để vẫn xóa frame khỏi map
	}
...
func handleFeature(frameNumber byte, sequenceNumber uint16, checksum uint16, payload []byte, addr *net.UDPAddr, conn *net.UDPConn, unixSocket string) error {
	// Giải mã payload
	if len(payload) < 7 {
		return fmt.Errorf("handleFeature - invalid payload length")
	}
	combinedData := append([]byte{FEATURE_FLAG}, payload...)

	uuid := string(payload[:6]) // UUID chiếm 6 bytes đầu tiên
	featureData := payload[6:]  // Dữ liệu còn lại là FeatureData
	...
	if !exists {
		return fmt.Errorf("handleFeature - frame not initialized for UUID: %s", uuid)
	}

	// Lock frame để đảm bảo không có goroutine khác truy cập vào frame.UUID hoặc frame.FeatureData
	frame.mu.Lock()
	defer frame.mu.Unlock()

	// Kiểm tra UUID
	if uuid != frame.UUID {
		return fmt.Errorf("handleFeature - frame UUID mismatch: received UUID: %s, expected UUID: %s", uuid, frame.UUID)
	}

	// Tính lại checksum để xác minh dữ liệu
	calculatedChecksum := calculateChecksum(frameNumber, sequenceNumber, combinedData, uuid)
	if calculatedChecksum != checksum {
		return fmt.Errorf(
			"handleFeature - checksum mismatch for sequence %d, received: %d, expected: %d",
			sequenceNumber, calculatedChecksum, checksum,
		)
	}

	// Lưu dữ liệu feature vào Frame
	frame.FeatureData = featureData

	// log.Printf("Feature data saved for frame UUID: %s", uuid)

	// Phản hồi thành công tới client
	_, err := conn.WriteTo([]byte("END"), addr)
	if err != nil {
		return fmt.Errorf("handleFeature - failed to send acknowledgment: %v", err)
	}

	// Thêm vào channel trước khi bắt đầu xử lý
	frameProcessingChan <- struct{}{}

	// Chạy processEndFeature trong goroutine riêng
	go func() {
		defer func() {
			<-frameProcessingChan // Giải phóng slot xử lý khi hoàn thành
		}()

		if err := processEndFeature(frameNumber, frame, unixSocket); err != nil {
			log.Printf("Error in processEndFeature: %v", err)
		}
	}()
...
func handlePacket(data []byte, addr *net.UDPAddr, conn *net.UDPConn, unixSocket string, port int) {
	...
	if sequenceNumber > 0 {
		if err := handleChunk(frameNumber, sequenceNumber, checksum, payload, addr, conn, port); err != nil {
	...
	case END_FLAG:
		if err := handleEnd(frameNumber, sequenceNumber, checksum, encryptedPayload, addr, conn, port); err != nil {
			log.Printf("Error handling end: %v", err)
		}
	case FEATURE_FLAG: // Thêm xử lý cho FEATURE_FLAG
		if err := handleFeature(frameNumber, sequenceNumber, checksum, encryptedPayload, addr, conn, unixSocket); err != nil {
			log.Printf("Error handling feature: %v", err)
		}
	default:
...
func StartServer(ramdisk string, unixSocket string, server_port int) {
	// Kiểm tra RAMDisk
	if !checkLinuxRamDisk(ramdisk) {
		log.Println("Không có RAMDisk đã được mount.")
		return
	}

	// Kiểm tra UnixSocket
	if !checkUnixSocket(unixSocket) {
		log.Println("Không có unixSocket được cài.")
		return
	}
	...
	log.Printf("Server started on %s with unixSocket %s", addr.String(), unixSocket)
	...
	for {
		...
		go handlePacket(packet, clientAddr, conn, unixSocket, server_port)
	}
}

func main() {
	ramdisk := "/mnt/ramdisk"              // Khởi tạo ramdisk cho Linux
	unixSocket := "/mnt/ramdisk/ai_socket" // Khởi tạo unixSocket cho Linux

	StartServer(ramdisk, unixSocket, 5050) // Gọi hàm start_server để khởi động server
}
