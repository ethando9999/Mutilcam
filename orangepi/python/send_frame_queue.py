...
async def process_frame_queue(frame_queue, stop_event):
    """
    Xử lý và đẩy frame vào queue dưới dạng async.
    """
    filename = "python/file.webp"
...
async def main():
    frame_queue = asyncio.Queue(maxsize=1000)  # Dùng asyncio.Queue cho async
    server_address = (SERVER_IP, SERVER_PORT)
    udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_socket.setblocking(True)
    udp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 4 * 1024 * 1024)
    ...
    try:
        log_cpu_temp()
        # Chạy đồng thời hai coroutine
        await asyncio.gather(
            process_frame_queue(frame_queue, stop_event),
