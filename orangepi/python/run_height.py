...
def create_modules_imports():
    ...
    return create_database, TrackingManager, config, start_socket_sender, TableAIClient

create_database, TrackingManager, config, start_socket_sender, TableAIClient = create_modules_imports()
...
async def main(args):
    ...
    # Khởi tạo TableAIClient để gửi dữ liệu chiều cao lên blockchain
    try:
        table_client = TableAIClient(disable_tls=ID_CONFIG.get("DISABLE_TLS", True))
    ...
    try:
        ...
        # 5. TableAIClient run
        all_tasks.append(asyncio.create_task(table_client.run(height_queue)))
