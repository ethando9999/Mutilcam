    ...
    try:
        ...
        if system == "Linux":
            ...
            if os.path.exists("/usr/bin/vcgencmd"):
                ...
                logger.info(f"CPU Temperature (Raspberry Pi): {temp}")
            else:
                # Debian/Ubuntu
                ...
                logger.info(f"CPU Temperature (Linux): {temp_c:.1f}°C")
        elif system == "Darwin":
            logger.info(f"CPU Thermal Level (macOS): n/a")
        else:
            logger.warning(f"Unsupported system: {system}. Unable to log CPU temperature.")
...
def send_init_sequence(udp_socket, frame_number, uuid, chunk_size, total_chunks, server_address):
    ...
    """
    # logger.info(f"Start init sequence with UUID {uuid} to server.")
    sequence_number = 0
    encrypted_payload = (
    ...
    # logger.info(f"Successfully sent init message to {server_address} for frame number: {frame_number}") 
    ...
    try:
        ...
        logger.error("Timeout waiting for server acknowledgment. Aborting frame.")
    return False
        ...
        while time.time() - start_time < 5.0:
            try:
                ...
                if data == b"SUCCESS":
                    ...
                    logger.warning(f"Failed to send frame {frame_number}, try resend failed chunks") 
...
def resend_failed_chunks(udp_socket, frame_number, uuid, frame_size, failed_sequences, sent_chunks, combined_checksums, server_address):
    ...
    for sequence in failed_sequences:
        if sequence in sent_chunks:
            ...
            logger.error(f"Missing sequence {sequence}. Cannot resend.")
    ...
    # logger.info("Sent end flag after resending failed chunks.")
...
def adjust_chunk_size(frame_size):
    ...
    if CHUNK_SIZE != MIN_CHUNK_SIZE:
        logger.info(f"CHUNK_SIZE changed to {CHUNK_SIZE} to fit frame within sequence limit.")
...
async def send_frame_queue(udp_socket, server_address, processed_frame_queue):
    ...
    while True:
        try:
            ...
            feature_data_bytes = processed_data["feature"]
            ...
            if current_time - last_fps_time >= 1:
                ...
                logger.info(f"FPS {server_address[1]}: {fps:.2f}")
                sent_frames = 0
...
async def start_sender(frame_queue, server_address):
    """
