        ...
        try:
            ...
            for i, box in enumerate(mask_boxes):
                try:
                    ...
                    human_box = main_frame[y1:y2, x1:x2]
                    ...
                    logging.info(f"Compressed human box size: {compressed_size/1024:.2f}KB, Compression ratio: {original_size/compressed_size:.2f}x")
                    
                    # Sử dụng try_put để tránh blocking
                    try:
                        await asyncio.wait_for(frame_queue.put(encoded_bytes), timeout=0.5)
                        logging.debug(f"Đã đưa frame {i+1} vào queue thành công")
                    except asyncio.TimeoutError:
                        logging.warning("Timeout khi đưa frame vào queue")
                        continue
...
async def main():
    ...
    try:
        ...
        sender_task = asyncio.create_task(
            send_frame_queue(udp_socket, server_address, frame_queue)
        )
