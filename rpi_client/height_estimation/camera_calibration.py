...
def calibrate_stereo_camera(folder_left, folder_right, number_pattern=40, pattern_size=(9, 6), square_size=2.8):
    """
    ...
    Parameters:
      ...
      - number_pattern: Số lượng ảnh mẫu tối đa dùng để hiệu chuẩn
    ...
    # Lấy kích thước ảnh từ ảnh cuối (giả định không resize)
    ...
    baseline_calib = np.linalg.norm(T)  # Baseline tính được (m)

    print("Baseline (m) hiệu chuẩn: ", baseline_calib)
    print("Tiêu cự (px) theo hiệu chuẩn: ", focal_length_calib)

    # Điều chỉnh tiêu cự nếu baseline thực tế khác baseline đã hiệu chuẩn (ví dụ: baseline thực là 7cm)
    real_baseline = 0.07  # 7 cm
    ...
    np.savez('calibration_data.npz', 
             ...
             scale=scale, focal_length_calib=focal_length_calib, focal_length_corrected=focal_length_corrected)

    print("Hiệu chuẩn hoàn tất và các tham số đã được lưu vào 'calibration_data.npz'.")

# Ví dụ sử dụng hàm:
if __name__ == "__main__":
    folder_cam_left = 'calibration4/cam1'
    folder_cam_right = 'calibration4/cam2'
    calibrate_stereo_camera(folder_cam_left, folder_cam_right, number_pattern=40, pattern_size=(9, 6), square_size=2.8)
